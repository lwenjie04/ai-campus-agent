...
const requiredFiles = [
  ...
  'docs/competition-rehearsal-log.md',
  'docs/competition-freeze-handoff.md',
]

const errors = []
const warnings = []
const strictConfig = process.env.COMPETITION_STRICT_CONFIG === 'true'
...
if (llmProviderMode === 'mock') {
  configIssue('LLM_PROVIDER_MODE=mock; this is suitable for automated rehearsal only, not the live competition claim')
...
if (!adminPassword) {
  ...
  errors.push('AUTH_DEFAULT_ADMIN_PASSWORD is still weak; set a competition-only strong password')
}
...
if (guestCookieSecret.length < 32) {
  configIssue('GUEST_COOKIE_SECRET is missing or shorter than 32 characters; set a stable competition secret')
}
...
if (!existsSync(resolve(process.cwd(), 'server/data/vector-index.json'))) {
  configIssue('vector index is missing; vector retrieval is not competition-ready yet')
}

for (const warning of warnings) console.warn(`[competition-assets] WARN ${warning}`)
for (const error of errors) console.error(`[competition-assets] FAIL ${error}`)

if (errors.length > 0) {
  process.exitCode = 1
} else {
  console.log(`[competition-assets] PASS ${requiredFiles.length} required files, ${warnings.length} warnings`)
}
